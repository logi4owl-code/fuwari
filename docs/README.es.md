...
2. Ejecuta `bun run new-post <nombre-de-archivo>` para crear una nueva entrada y edítala en `src/content/posts/`.
...
2. Para editar tu blog localmente, clona tu repositorio, ejecuta `bun install` para instalar las dependencias.
   - Instala [bun](https://bun.sh) si aún no lo tienes.
3. Edita el archivo de configuración `src/config.ts` para personalizar tu blog.
4. Ejecuta `bun run new-post <nombre-de-archivo>` para crear una nueva entrada y edítala en `src/content/posts/`.
...
| `bun run build`                         | Compila tu web para producción en `./dist/`     |
| `bun run preview`                       | Previsualiza la web localmente, antes del despliegue |
| `bun run new-post <nombre-de-archivo>`  | Crea una nueva entrada                            |
| `bun run astro ...`                     | Ejecuta comandos CLI como `astro add`, `astro check` |
| `bun run astro --help`                  | Obtén ayuda para usar el CLI de Astro             |
