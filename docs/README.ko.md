...
4. `bun run new-post <filename>`을 실행하여 새 게시물을 만들고 `src/content/posts/`에서 수정하세요.
...
| Command                             | Action                                           |
|:------------------------------------|:-------------------------------------------------|
| `bun install`                       | 종속성을 설치합니다.                            |
| `bun run dev`                       | `localhost:4321`에서 로컬 개발 서버를 시작합니다.      |
| `bun run build`                     | `./dist/`에 프로덕션 사이트를 구축합니다.         |
| `bun run check`                     | 코드에서 오류를 확인합니다.         |
| `bun run format`                    | Biome을 사용하여 코드를 포멧합니다.         |
| `bun run preview`                   | 배포하기 전에 로컬에서 빌드 미리보기     |
| `bun run new-post <filename>`       | 새 게시물 작성                                |
| `bun run astro ...`                 | `astro add`, `astro check`와 같은 CLI 명령어 실행 |
| `bun run astro --help`              | Astro CLI를 사용하여 도움 받기                     |
