...
2. 执行 `bun run new-post <filename>` 创建新文章，并在 `src/content/posts/` 目录中编辑
...
2. 进行本地开发，Clone 新的仓库，执行 `bun install` 以安装依赖  
   - 若未安装 [bun](https://bun.sh)，请参考官网安装指南
3. 通过配置文件 `src/config.ts` 自定义博客
4. 执行 `bun run new-post <filename>` 创建新文章，并在 `src/content/posts/` 目录中编辑
...
| `bun run astro ...`               | 执行 `astro add`, `astro check` 等指令 |
| `bun run astro --help`            | 显示 Astro CLI 帮助                   |
